...
void *Encoder_Thread(void *arg)
{
	...
	while (hv->encoder.encoder_context.frame_count <= hv->detect.frame_num)
	{
       ...
       if (hv->decoder.flag_decoder == 0) {
		    ...
		    ret = disp_video_layer_param_set(fbAddr, VIDEO_PIXEL_FORMAT_NV21, &win, 1);
        }
#endif
...
#ifdef DISPLAY_SOURCE_DATA
            fbAddr[0] = (unsigned int)hv->decoder.Phy_Y_Addr;
            fbAddr[1] = (unsigned int)hv->decoder.Phy_C_Addr;
		    ret = disp_video_layer_param_set(fbAddr, VIDEO_PIXEL_FORMAT_NV21, &win, 1);
#endif
			/* VIDIOC_QBUF qbuf */
			if (ioctl(hv->detect.videofd, VIDIOC_QBUF, &buf_pop) == 0)
